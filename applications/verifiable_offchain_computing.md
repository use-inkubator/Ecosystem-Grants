...
The main goal is to provide the same features as [Phala's offchain rollup anchor](https://github.com/Phala-Network/phat-offchain-rollup) but with the latest technologies (ink! v6 and Phala Cloud compatibility)
...
- For the VRF Oracle, we will use it to implement the game Guess The Number.
- For the Graph API Oracle, we will index the data from the game Guess The Number previously developed.  
...
### Milestone 4 - Graph API Oracle + Support for ink! v6 contract indexing in Subsquid (only Events generated by the contracts will be supported - the contract calls are out of scope)
